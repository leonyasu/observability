...
def put_execution_data(dimension, value):
    ...
    if ( value > 8):
        message = dimension + ":"+ str(value) + " - Above 8s"
    else:
        message = dimension + ":"+ str(value) + " - Bellow 8s"
...
def put_Log(message):
    ...
    #    logGroupName='bedrock_app',
    #    logStreamName='llm_calls'
    #)
...
def main():
    ...
    # Start a segment
    segment = xray_recorder.begin_segment('llms')
    
    # Start a subsegment
    subsegment = xray_recorder.begin_subsegment('sonnet')
    # Add metadata and annotations
    segment.put_metadata('key', dict, 'namespace')
    subsegment.put_annotation('key', 'sonnet')
    ...
    # Start a subsegment
    subsegment1 = xray_recorder.begin_subsegment('haiku')
    # Add metadata and annotations
    segment.put_metadata('key', dict, 'namespace')
    subsegment1.put_annotation('key', 'haiku')
    invoke_claude_v3_haiku("Teste")
    # Close the subsegment and segment
    xray_recorder.end_subsegment()
    
     # Start a subsegment
    subsegment2 = xray_recorder.begin_subsegment('haiku')
    # Add metadata and annotations
    segment.put_metadata('key', dict, 'namespace')
    subsegment2.put_annotation('key', 'haiku')
    invoke_claude_v3_haiku("Teste 2")
    # Close the subsegment and segment
    xray_recorder.end_subsegment()
